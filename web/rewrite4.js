...
export default function QRScanner() {
  ...
  return (
    <div className="relative min-h-[100dvh] bg-black flex flex-col items-center justify-center overflow-hidden selection:bg-transparent">
      <button 
        onClick={() => router.push("/")}
        className="absolute top-6 left-6 z-50 p-3 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full text-white transition-all shadow-lg active:scale-95 border border-white/10"
      >
      ...
      <div className="w-full h-[100dvh] absolute inset-0 flex items-center justify-center pointer-events-auto">
        <div
          id="qr-reader"
          className="w-full h-full [&_video]:object-cover border-none bg-black"
        />
      </div>
      ...
      {!statusMessage && (
        <div className="absolute bottom-16 z-20 pointer-events-none flex flex-col items-center gap-4">
          ...
          <div className="bg-black/50 backdrop-blur-md px-10 py-5 rounded-full border border-white/10 shadow-2xl">
